            ...
            <CardContent className="p-5 flex items-center gap-5 relative z-10 w-full">
                ...
                <div className="flex-1">
                    <div className="flex justify-between items-end mb-2">
                        <div>
                            <h3 className="text-lg font-extrabold text-slate-100 leading-tight">Level {profile.level}</h3>
                            <p className="text-xs font-medium text-slate-500 flex items-center gap-1">
                                <Star className="w-3.5 h-3.5 text-[#D4AF37] fill-[#D4AF37]" />
                                {profile.points} Total Points
                            </p>
                        </div>
                        <p className="text-xs font-bold text-slate-400 bg-white/10 px-2 py-1 rounded-lg">
                ...
                <div className="bg-white/5 border-t border-white/10 px-5 py-3 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        {profile.badges.slice(-3).map((badge: any) => (
                            <div key={badge.id} className="w-6 h-6 rounded-full bg-white/10 border border-white/10 flex items-center justify-center text-xs hover:scale-110 transition-transform cursor-pointer" title={badge.name}>
                                {badge.icon}
                            </div>
                        ))}
                        {profile.badges.length > 3 && (
                            <span className="text-xs font-bold text-slate-500">+{profile.badges.length - 3}</span>
                        )}
                    </div>
                </div>
            )}
