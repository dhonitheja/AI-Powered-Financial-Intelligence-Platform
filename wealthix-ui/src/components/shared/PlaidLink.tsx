...
const PlaidLink: React.FC<PlaidLinkProps> = ({ onSuccess, userId = "default-user" }) => {
    ...
    if (connectionStatus === 'connected') {
        return (
            <div className="flex items-center gap-2 text-emerald-400 font-bold text-sm bg-emerald-500/15 px-4 py-2 rounded-xl border border-emerald-500/30">
    ...
    return (
        <div className="flex flex-col items-start gap-2">
            <button
                ...
                className="bg-gradient-to-r from-[#D4AF37] to-[#B8962E] text-[#0D0B1E] px-6 py-2.5 rounded-xl font-bold text-sm shadow-gold flex items-center gap-2 transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 disabled:scale-100"
            >
