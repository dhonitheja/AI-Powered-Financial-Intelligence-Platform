...
export function AutoPayCard({ schedule, onToggle, onDelete, onExecute, onClick }: AutoPayCardProps) {
    ...
    return (
        <div
            className="group relative glass-obsidian rounded-2xl p-6 hover:shadow-[0_0_40px_rgba(59,130,246,0.15)] hover:border-blue-500/30 transition-all duration-500 cursor-pointer overflow-hidden"
            onClick={() => onClick(schedule.id)}
        >
            {/* Status indicator strip */}
            {schedule.active && (
                <div
                    className={`absolute left-0 top-6 bottom-6 w-1 rounded-r-full shadow-[0_0_10px_rgba(34,197,94,0.4)] ${isOverdue ? 'bg-red-500' : isDueSoon ? 'bg-amber-400' : 'bg-green-500 pulse-active'
                        }`}
                />
            )}
