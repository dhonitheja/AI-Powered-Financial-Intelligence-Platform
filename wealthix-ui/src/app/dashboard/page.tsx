    ...
    const fetchData = useCallback(async (activePeriod: Period) => {
        ...
        try {
            const results = await Promise.allSettled([
                ...
                plaidService.getAccountSummary()
            ]);

            // 1. Transactions
            if (results[0].status === 'fulfilled') {
                const txRes = results[0].value;
                ...
                console.error('[Dashboard] Failed to fetch transactions:', results[0].reason);
                setTransactions([]);
            }

            // 2. Summary
            if (results[1].status === 'fulfilled') {
                const summaryRes = results[1].value;
                setSummary(Array.isArray(summaryRes.data) ? summaryRes.data : []);
            } else {
                console.error('[Dashboard] Failed to fetch summary:', results[1].reason);
                setSummary([]);
            }

            // 3. Accounts
            if (results[2].status === 'fulfilled') {
                const accountRes = results[2].value;
                if (accountRes?.data) setFinancialSummary(accountRes.data);
            } else {
                console.error('[Dashboard] Failed to fetch accounts:', results[2].reason);
            }

        } catch (error: any) {
            console.error('[Dashboard] Unexpected fetch error:', error);
    ...
    return (
        <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-700">
            ...
            <div className="flex flex-wrap justify-between items-end gap-4">
                <div>
                    <h1 className="text-3xl font-black text-slate-100 tracking-tight">AI Finance Assistant</h1>
                    <p className="text-slate-500 mt-1 font-medium">Intelligent insights powered by Gemini AI.</p>
                </div>
                <div className="flex items-center gap-3">
                    {/* Period Dropdown */}
                    <div className="relative flex items-center gap-2 bg-white/5 border border-white/10 rounded-xl px-4 py-2.5">
                        <CalendarDays className="w-4 h-4 text-[#D4AF37] shrink-0" />
                        <select
                            value={period}
                            onChange={(e) => setPeriod(e.target.value as Period)}
                            className="appearance-none bg-transparent text-sm font-bold text-slate-200 pr-6 focus:outline-none cursor-pointer"
                        >
                    ...
                    <button
                        ...
                        className="bg-gradient-to-r from-[#D4AF37] to-[#B8962E] text-[#0D0B1E] px-5 py-2.5 rounded-xl font-bold shadow-gold hover:scale-105 active:scale-95 transition-all"
                    >
                        Add Transaction
                    </button>
                </div>
            ...
            >
                <div className="overflow-x-auto mt-4 max-h-[520px] overflow-y-auto">
                    <table className="w-full text-left">
                        <thead className="sticky top-0 bg-[#0D0B1E] z-10 border-b border-white/10 uppercase text-[10px] tracking-widest text-slate-500 font-bold">
                            <tr>
...
function StatCard({ title, value, icon, badge, isPositive }: any) {
    return (
        <Card className="hover:border-[#D4AF37]/30 transition-all duration-300">
            <div className="flex justify-between items-start">
                ...
                <div className={cn(
                    ...
                    isPositive ? "text-emerald-400 bg-emerald-500/10 border border-emerald-500/20" : "text-amber-400 bg-amber-500/10 border border-amber-500/20"
                )}>
                    {badge}
                </div>
            </div>
            <div className="mt-4">
                ...
                <h4 className="text-2xl font-black text-slate-100 mt-1">{value}</h4>
            </div>
        </Card>
    );
}
    ...
    return (
        <tr className="hover:bg-white/5 transition-colors group cursor-pointer" onClick={onClick}>
            <td className="py-4 px-4">
                <p className="font-bold text-sm text-slate-200">{description}</p>
                <p className="text-[10px] text-slate-500 uppercase tracking-tighter font-medium">{date}</p>
            </td>
            <td className="py-4 px-4">
                <span className="px-2 py-1 rounded-md bg-white/5 text-slate-400 text-[9px] font-black uppercase tracking-wider border border-white/10">{category}</span>
            </td>
            <td className={cn("py-4 px-4 font-black text-sm flex items-center gap-1", amount < 0 ? "text-slate-200" : "text-emerald-400")}>
                            ...
                            {bankAccounts.map((acc: any, i: number) => (
                                <div key={i} className="flex items-center justify-between p-3 bg-white/5 rounded-xl border border-white/10">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="font-bold text-sm text-slate-200">{acc.accountName || acc.institutionName}</p>
                                            <p className="text-[10px] text-slate-500 uppercase font-bold">{acc.accountType}</p>
                                        </div>
                                    </div>
                                ...
                                return (
                                    <div key={i} className="p-3 bg-white/5 rounded-xl border border-white/10 space-y-2">
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-3">
                                                <div className="w-9 h-9 rounded-xl bg-rose-500/10 text-rose-400 flex items-center justify-center text-[9px] font-black">CC</div>
                                                <div>
                                                    <p className="font-bold text-sm text-slate-200">{acc.accountName || acc.institutionName}</p>
                                                    <p className="text-[10px] text-slate-500 uppercase font-bold">Credit Card</p>
                                                </div>
                                            </div>
                                            <div className="text-right">
...
function NetWorthPill({ label, value, color, highlight }: { label: string; value: string; color: string; highlight?: boolean }) {
    const styles: Record<string, string> = {
        ...
        teal: 'border-[#D4AF37]/30 bg-[#D4AF37]/10 text-[#D4AF37]',
    };
    return (
