        ...
        private void runMigrations() {
                try {
                        ...
                        if (!"PostgreSQL".equalsIgnoreCase(dbProduct)) {
                                log.info("Skipping PostgreSQL-specific migrations for database: {}", dbProduct);
                                return;
                        }
                        ...
                        jdbcTemplate.execute(
                                        ...
                                        ")");
                        log.info("✔ Gamification tables ensured.");
                        ...
                        jdbcTemplate.execute(
                                        ...
                                        "  created_at TIMESTAMPTZ NOT NULL DEFAULT now()," +
                                        "  updated_at TIMESTAMPTZ NOT NULL DEFAULT now()," +
                                        "  CONSTRAINT uq_budget_user_category_period UNIQUE (user_id, category, period)" +
                                        ")");
                        log.info("✔ Category budgets table ensured.");
